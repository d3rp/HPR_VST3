...
if %1 == 1 (echo Making hpr Windows DEMO VERSION distribution ...) else (echo Making hpr Windows FULL VERSION distribution ...)
...
REM - msbuild hpr-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild hpr.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild hpr.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\hpr.aaxplugin\Contents\Win32\hpr.aaxplugin --out .\build-win\aax\bin\hpr.aaxplugin\Contents\Win32\hpr.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\hpr.aaxplugin\Contents\x64\hpr.aaxplugin --out .\build-win\aax\bin\hpr.aaxplugin\Contents\x64\hpr.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\hpr.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\hpr.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "hpr Installer" ".\installer\hpr Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\hpr Installer.exe" ".\installer\hpr Demo Installer.exe"
REM -del ".\installer\hpr Installer.exe"
REM -)
