...
def  main():
  ...
  installer = "\installer\hpr Installer.exe"
   
  if demo:
    installer = "\installer\hpr Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\hpr manual.pdf" 
  ]
  ...
  ZIPNAME = "hpr-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "hpr-v" + FULLVERSIONSTR + "-win-demo.zip"
